...
def data_preparing(path, stop_at, perc, seed=0):
    ...
    # training set
    training_x = training_trim.iloc[:, 1:-1]  # pandas.core.frame.DataFrame
    training_y = training_trim.iloc[:, -1]  # pandas.core.series.Series

    # testing set
    testset_x = testing_trim.iloc[:, 1:-1]
    testset_y = testing_trim.iloc[:, -1]

    training_x.drop(['user_comments', 'system_records',
                     's7'], axis=1, inplace=True)
    testset_x.drop(['user_comments', 'system_records',
                    's7'], axis=1, inplace=True)

    training_x['s90'] = [eval(x)[0] for x in training_x['s9']]
    testset_x['s90'] = [eval(x)[0] for x in testset_x['s9']]
    training_x['s91'] = [eval(x)[1] for x in training_x['s9']]
    testset_x['s91'] = [eval(x)[1] for x in testset_x['s9']]
    training_x['s92'] = [eval(x)[2] for x in training_x['s9']]
    testset_x['s92'] = [eval(x)[2] for x in testset_x['s9']]

    training_x.drop('s9', axis=1, inplace=True)
    testset_x.drop('s9', axis=1, inplace=True)
...
def main(config):
    ...
    stopats = [1]
    for project in ['firefox', 'chromium', 'eclipse']:
        ...
        cost = []
        Acc = []
        training_data = data_preparing(path, stop_at=1, perc=0,
